...
$res=mysql_query("select id, user_name from vtiger_users where deleted=0 and  user_name !=  'alexcorp' and user_name !=  'Giselle' and user_name !=  'ivan' and user_name !=  'admin' order by user_name");
...
$msort =$_POST['cf_734'];
if($msort!="")
$msort = "and lcf.cf_734='".$_POST['cf_734']."'";
...
//BNJ added cf_735 below
	...
	$query="select mc.modcommentsid,mc.commentcontent,crm.smownerid, l.leadid, l.firstname, l.lastname, lcf.cf_641 dba,lcf.cf_734,crm.modifiedtime, crm.createdtime,lcf.cf_735, lcf.cf_736 as stat_date,l.leadstatus
	from vtiger_leaddetails l 
	inner join vtiger_leadscf lcf on l.leadid=lcf.leadid
	inner join vtiger_crmentity crm on l.leadid=crm.crmid inner join  vtiger_modcomments mc ON mc.related_to = l.leadid where crm.deleted=0  AND l.converted =0";
	$query="select crm.smownerid, l.leadid, l.firstname, l.lastname, lcf.cf_641 dba,lcf.cf_734,crm.modifiedtime, crm.createdtime,lcf.cf_735, lcf.cf_736 as stat_date,l.leadstatus
	from vtiger_leaddetails l 
	inner join vtiger_leadscf lcf on l.leadid=lcf.leadid
	inner join vtiger_crmentity crm on l.leadid=crm.crmid where crm.deleted=0  AND l.converted =0";
	$query.=" and crm.modifiedtime   >='".$startdaten." 00:00:00'";
	$query.=" and crm.modifiedtime   <='".$enddaten." 00:00:00' ".$msort." ";
	//if (count($_REQUEST['user'])>0) $query.=" and crm.smownerid in (".  implode(", ", $_REQUEST['user']).")";
	//$query.="  order by mc.modcommentsid desc limit 1";
...
file_put_contents("wtfwtf.txt", $query);
	...
	while($row=  mysql_fetch_assoc($res)){
		...
		$res2=mysql_query("Select mc.modcommentsid,mc.commentcontent, crm.createdtime FROM vtiger_modcomments mc
                   INNER JOIN vtiger_crmentity crm ON mc.modcommentsid = crm.crmid
                   INNER JOIN vtiger_users u ON crm.smownerid = u.id
                    WHERE related_to =".$row['leadid']." order by mc.modcommentsid desc limit 1");
                   /*  $res2=mysql_query("Select mc.modcommentsid,mc.commentcontent, crm.createdtime FROM vtiger_modcomments mc, vtiger_crmentity crm, vtiger_users u
                   where mc.modcommentsid = crm.crmid
                   and crm.smownerid = u.id l
                    and related_to =".$row['leadid']." order by mc.modcommentsid desc limit 1");	*/
				while ($row2 = mysql_fetch_array($res2))
				{
				$lnote =$row2['commentcontent'];
			    $correctd=$row2['createdtime'];
				}
		
		if ((($correctd >= $startdaten) && ($correctd <= $enddaten)) || ($row['createdtime'] >= $startdaten &&  $row['createdtime']<= $enddaten) )
		{
		$restable[$row['smownerid']][]=array(
			...
			"dba"=>$row['dba'],//$correctd
			//"assigned"=>$correctd,
			"assigned"=>$row['modifiedtime'],
...
<tr><td><select name='cf_734' id='src' tabindex=''> 
	<option value='' > Choose a Campaign </option> 
	<option value='Incoming Calls' > Incoming Calls </option> 
	<option value='Outbound Calls' > Outbound Calls </option> 
	<option value='Emails' > Email </option> <option value='Paper Leads' > Paper Leads </option> 
	<option value='Press1' > Press1 </option> <option value='Auto Dialer' > Auto Dialer </option> 
...
foreach($restable as $key=>$value){
	...
	<td class=\"lvtCol\" style='width:8%'>Last Note</td>
	...
	<td><a href=index.php?module=Leads&parenttab=Marketing&action=DetailView&record=".$value1['leadid']." target='blank'>".$value1['dba']."</a></td>
	...
	echo"<td onmouseover=\"vtlib_listview.trigger('cell.onmouseover', $(this))\" onmouseout=\"vtlib_listview.trigger('cell.onmouseout', $(this))\">".$value1['note']."</td>";
	echo"</tr>";
