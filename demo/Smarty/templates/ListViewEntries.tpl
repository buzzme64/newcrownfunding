...
str = str.replace(/<td .*?><a href=.*?>(.+?)<\/a>\s+<span.*?cf_648.*?<\/span><\/td>/g, "<td><a href=\"tel:$1\">$1<\/a><\/td>");
newWin.document.write(str);'>monkey</a>
			...
			{foreach item=entity key=entity_id from=$LISTENTITY}
				{if $MODULE eq 'Leads'}
					{assign var="trstyle" value=""}
					{foreach item=tr key=changeid from=$FULL}
						{if $changeid eq $entity_id}
							{assign var="trstyle" value=$tr}
						{/if}
					{/foreach}
				{else}
					{assign var="trstyle" value=""}
				{/if}
			<tr bgcolor=white onMouseOver="this.className='lvtColDataHover'" onMouseOut="this.className='lvtColData'" id="row_{$entity_id}"{$trstyle}>
			...
			{if $MODULE eq 'Leads' && $VIEWNOTES eq 'on'}
				<tr bgcolor=LightGray onMouseOver="this.className='lvtColDataHover';this.style.backgroundColor='';" onMouseOut="this.className='lvtColData';this.style.backgroundColor='LightGray';">
					<td colspan="{$COUNTFIELD}">
						{foreach item=lastnotes from=$VIEWLASTNOTES}
							{if $lastnotes.related_to eq $entity_id}
								<div id="viewnotes{$entity_id}">
									<div class="dataField" valign="top" style="width: 99%; padding-top: 10px;">{$lastnotes.commentcontent}</div>
									<div class="dataLabel" valign="top" style="border-bottom: 1px dotted rgb(204, 204, 204); width: 99%; padding-bottom: 5px;">
									<font color="darkred"> Author: {$lastnotes.first_name} {$lastnotes.last_name} on {$lastnotes.createdtime} </font>
									</div>
								</div>
								<div id="addnotes{$entity_id}" style="display:none;">
									<textarea id="newnote{$entity_id}" class="detailedViewTextBox" onfocus="this.className='detailedViewTextBoxOn'" onblur="this.className='detailedViewTextBox'" rows="8" style="width:100%"></textarea>
								</div>
								<div id="panelnotes">
									<span id="spanaddnote{$entity_id}"><input type='button' value='Add note' class="crmbutton small edit" onclick="addnote('{$entity_id}')" /></span>
									<span id='spansave{$entity_id}' style="display:none;">
										<input type="button" class="crmbutton small edit" onclick="javascript:savenote('{$entity_id}')" value="Save" />
										or
										<input type="button" class="crmbutton small delete" onclick="javascript:cancelsavenote('{$entity_id}')" value="Cancel" />
									</span>
								</div>
							{/if}
						{/foreach}
