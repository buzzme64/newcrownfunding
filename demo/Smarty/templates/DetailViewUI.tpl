...
<!-- This file is used to display the fields based on the ui type in detailview -->
		{if $keyid eq '1' || $keyid eq 2 || $keyid eq '11' || $keyid eq '7' || $keyid eq '9' || $keyid eq '55' || $keyid eq '71' || $keyid eq '72' || $keyid eq '103' || $keyid eq '255' || $keyid eq '14'} <!--TextBox-->
			<td width=25% class="dvtCellInfo" align="left" id="mouseArea_{$label}" onmouseover="hndMouseOver({$keyid},'{$label}');" onmouseout="fnhide('crmspanid');" valign="top"{$tdstyle}>
				{if $keyid eq '55' || $keyid eq '255'}<!--SalutationSymbol-->
					...
					{else}
						<span style="display: none;">{$keysalut}</span>
					{/if}
				{/if}
        ...
        {elseif $keyid eq '13' || $keyid eq '104'} <!--Email-->
            <td width=25% class="dvtCellInfo" align="left" id="mouseArea_{$label}" onmouseover="hndMouseOver({$keyid},'{$label}');" onmouseout="fnhide('crmspanid');"{$tdstyle}><span id="dtlview_{$label}">
                ...
                <div id="editarea_{$label}" style="display:none;">
                	...
                	<br>
				{if $MODULE eq 'Leads'}
					<input name="button_{$label}" type="button" class="crmbutton small save" value="{$APP.LBL_SAVE_LABEL}" onclick="SaveEmail('{$label}','{$MODULE}',{$keyfldid},'{$keytblname}','{$keyfldname}','{$ID}');fnhide('crmspanid');"/> 
				{else}
	...
	{elseif $keyid eq '15' || $keyid eq '16' || $keyid eq '31' || $keyid eq '32'} <!--ComboBox-->
		{if $MODULE eq 'Leads' && $label eq 'Lead Status'}
			<td class="dvtCellInfo" align="left" id="mouseArea_{$label}" onmouseover="hndMouseOver({$keyid},'{$label}');" onmouseout="fnhide('crmspanid');"{$tdstyle}>
				{*<table>*}
			...
			{*<td>*}<input type="radio" {if $arr[2] eq 'selected'}checked=""{else}readonly="" disabled=""{/if} />{$arr[0]}{*</td>*}
					{*if $i eq 4}</tr>{assign var=i value='0'}{/if*}
				{/foreach}
				</span>
				{*</tr>
				</table>*}
		{else}
						{foreach item=arr from=$keyoptions}
							...
							{/if}
						{/foreach}
							<td width=25% class="dvtCellInfo" align="left" id="mouseArea_{$label}" onmouseover="hndMouseOver({$keyid},'{$label}');" onmouseout="fnhide('crmspanid');"{$tdstyle}><span id="dtlview_{$label}"><font color="{$fontval}">{if $APP.$keyval!=''}{$APP.$keyval}{elseif $MOD.$keyval!=''}{$MOD.$keyval}{else}{$keyval}{/if}</font></span>
		{/if}
													<div id="editarea_{$label}" style="display:none;">
		{if $MODULE eq 'Leads' && $keyid eq '15' && $label eq 'Lead Status'}
			{assign var='existselect' value=false}
			{assign var='appnotsend' value=false}
			{foreach item=arr from=$keyoptions}
				{if $arr[2] eq 'selected'}
					{assign var='existselect' value=true}
					{assign var='viewsave' value=$arr[0]}
					<input type="hidden" name="hidezone" value="{$arr[0]}" id="hidezone" />
					{if $arr[0] eq '1. App not sent' or $arr[0] eq 'Lead Pending' or $arr[0] eq '10. Follow ups'}
			...
			{foreach item=arr from=$keyoptions}
				{if $existselect eq false}
					{if $arr[0] eq '1. App not sent'}
						<input type='radio' name="{$keyfldname}" value="{$arr[0]}" checked="" onclick="changeleadstatus('{$arr[0]}')" />
					{else}
						<input type='radio' name="{$keyfldname}" value="{$arr[0]}" onclick="changeleadstatus('{$arr[0]}')" />
					{/if}
				...
				{else}
					{if $arr[0] eq '1. App not sent' && $existselect eq false}
						<input type='radio' name="{$keyfldname}" value="{$arr[0]}" checked="" onclick="changeleadstatus('{$arr[0]}',false)" />
					{else}
						{if $arr[0] eq '9. Declined' or $arr[0] eq 'Lead Pending' or $arr[0] eq '8. Merchant Pass' or $arr[0] eq '10. Follow ups'}
							<input type='radio' name="{$keyfldname}" value="{$arr[0]}"{if $arr[2] eq 'selected'} checked=""{/if} onclick="changeleadstatus('{$arr[0]}')" />
							{if $arr[2] eq 'selected'}
								{assign var='nextstep' value=false}
							{/if}
						{else}
							{if $arr[2] eq 'selected'}
								...
								<input type='radio' name="{$keyfldname}" value="{$arr[0]}"{if $currstep eq true} checked=""{/if} onclick="changeleadstatus('{$arr[0]}')" />
								{assign var='currstep' value=false}
							{elseif $existselect eq true}
								<input type='radio' name="{$keyfldname}" value="{$arr[0]}"{if $currstep eq true} checked=""{/if}{if $nextstep eq false} disabled=""{/if} onclick="changeleadstatus('{$arr[0]}')" />
								{assign var='nextstep' value=false}
							{else}
								<input type='radio' name="{$keyfldname}" value="{$arr[0]}"{if $nextstep eq false} disabled=""{/if} onclick="changeleadstatus('{$arr[0]}')" />
							{/if}
						{/if}
					{/if}
				{/if}
			...
			<input name="button_{$label}" type="button" class="crmbutton small save" value="{$APP.LBL_SAVE_LABEL}" onclick="updateleadstatus('{$ID}');{*dtlViewAjaxSave('{$label}','{$MODULE}',{$keyid},'{$keytblname}','{$keyfldname}','{$ID}');fnhide('crmspanid');*}"/> {$APP.LBL_OR}
			<a href="javascript:;" onclick="hndCancel('dtlview_{$label}','editarea_{$label}','{$label}')" class="link">{$APP.LBL_CANCEL_BUTTON_LABEL}</a>
			</div>
		{else}
		...
		{/if}
						...
						{elseif $keyid eq '116' || $keyid eq '117'} <!--ComboBox currency id edit only for admin Users-->
								...
								{/if}
                                             ...
                                             {elseif $keyid eq '17'} <!--WebSite-->
                                                  <td width=25% class="dvtCellInfo" align="left" id="mouseArea_{$label}" onmouseover="hndMouseOver({$keyid},'{$label}');" onmouseout="fnhide('crmspanid');"{$tdstyle}>&nbsp;<span id="dtlview_{$label}"><a href="{$keyval}" target="_blank">{$keyval}</a></span>
                                              		<div id="editarea_{$label}" style="display:none;">
						...
						{/if}
							...
							<!--To give hyperlink to URL-->
								...
								{$keyval|regex_replace:"/(^|[\n ])([\w]+?:\/\/.*?[^ \"\n\r\t<]*)/":"\\1<a href=\"\\2\" target=\"_blank\">\\2</a>"|regex_replace:"/(^|[\n ])((www|ftp)\.[\w\-]+\.[\w\-.\~]+(?:\/[^ \"\t\n\r<]*)?)/":"\\1<a href=\"http://\\2\" target=\"_blank\">\\2</a>"|regex_replace:"/(^|[\n ])([a-z0-9&\-_.]+?)@([\w\-]+\.([\w\-\.]+\.)*[\w]+)/i":"\\1<a href=\"mailto:\\2@\\3\">\\2@\\3</a>"|regex_replace:"/,\"|\.\"|\)\"|\)\.\"|\.\)\"/":"\""|replace:"\n":"<br>&nbsp;"}
																...
																{if $MODULE eq 'Leads'}
																<br><input name="button_{$label}" type="button" class="crmbutton small save" value="{$APP.LBL_SAVE_LABEL}" onclick="updateleadstatus('{$ID}');" {*onclick="dtlViewAjaxSave('{$label}','{$MODULE}',{$keyid},'{$keytblname}','{$keyfldname}','{$ID}');fnhide('crmspanid');"*}/> {$APP.LBL_OR}
                                                                <a href="javascript:;" onclick="hndCancel('dtlview_{$label}','editarea_{$label}','{$label}')" class="link">{$APP.LBL_CANCEL_BUTTON_LABEL}</a>
																{else}
                                                                <br><input name="button_{$label}" type="button" class="crmbutton small save" value="{$APP.LBL_SAVE_LABEL}" onclick="dtlViewAjaxSave('{$label}','{$MODULE}',{$keyid},'{$keytblname}','{$keyfldname}','{$ID}');fnhide('crmspanid');"/> {$APP.LBL_OR}
                                                                <a href="javascript:;" onclick="hndCancel('dtlview_{$label}','editarea_{$label}','{$label}')" class="link">{$APP.LBL_CANCEL_BUTTON_LABEL}</a>
                                             ...
                                             {elseif $keyid eq '21' || $keyid eq '24' || $keyid eq '22'} <!--TextArea/Street-->
                                                  <td width=25% class="dvtCellInfo" align="left" id="mouseArea_{$label}" onmouseover="hndMouseOver({$keyid},'{$label}');" onmouseout="fnhide('crmspanid');"{$tdstyle}>&nbsp;<span id="dtlview_{$label}">{$keyval}</span>
                                              		<div id="editarea_{$label}" style="display:none;">
			...
			{/if}
				...
				{elseif $keyid eq 5}
					...
					{/if}
					<td width=25% class="dvtCellInfo" align="left" id="mouseArea_{$label}" onmouseover="hndMouseOver({$keyid},'{$label}');" onmouseout="fnhide('crmspanid');"{$tdstyle}>
						&nbsp;&nbsp;<span id="dtlview_{$label}">
							{$keyval}
						</span>
						<div id="editarea_{$label}" style="display:none;">
