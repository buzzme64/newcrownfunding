...
class Documents extends CRMEntity {
	...
	{
		global $log,$adb,$upload_badext,$root_directory;
		...
		if($this->column_fields[$filetype_fieldname] == 'I' ){
			if($_REQUEST['dropFileName'] != ''){
				//$errCode=$_FILES[$filename_fieldname]['error'];
					//if($errCode == 0){
						//foreach($_FILES as $fileindex => $files)
						//{
							if($_REQUEST['dropFileName'] != '' && $_REQUEST['dropFileSize'] > 0){
								$filename = $_REQUEST['dropFileName'];
								$filename = from_html(preg_replace('/\s+/', '_', $filename));
								$filetype = $_REQUEST['dropFileType'];
								$filesize = $_REQUEST['dropFileSize'];
								$filelocationtype = 'I';
		...
		if($filelocationtype == 'I') {
			...
			$path2folder = decideFilePath();
			$this->uploadAndSaveFile($id,$module,$files);
			$current_id = $adb->getUniqueID("vtiger_crmentity") + 1;
			CRMEntity::insertIntoAttachment1($this->id, $module, '', $filename, $filesize, $filetype,$_REQUEST['assigned_user_id'],$path2folder);
			rename( $root_directory . 'drop/php/files/' . $_REQUEST['dropFileName'] , $root_directory . $path2folder . $current_id . "_" . $filename);
		}else{
...
//		echo $upload_dir;
//		echo '</pre>';
//		echo 'path2dropfolder: ' . $root_directory . 'drop/php/files/' . $_REQUEST['dropFileName']. '<br>';
//		echo 'path2storage: ' . $root_directory . $path2folder . $filename . '<br>';
//		echo 'filename: ' . $filename . '<br>';
//		echo 'filetype: ' . $filetype . '<br>';
//		echo 'current_id: ' . $adb->getUniqueID("vtiger_crmentity");
