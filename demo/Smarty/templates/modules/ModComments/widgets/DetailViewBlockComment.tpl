...
<td colspan="4" class="dvInnerHeader">
	...
	<span style="float: right;">
		...
		{$APP.LBL_SHOW} <select class="small" onchange="ModCommentsCommon.reloadContentWithFiltering('{$WIDGET_NAME}', '{$ID}', this.value, 'tbl{$UIKEY}', 'indicator{$UIKEY}');">
			...
			<option value="Last7" {if $CRITERIA eq 'Last7'}selected{/if}>{$MOD.LBL_LAST7}</option>
...
<div id="tbl{$UIKEY}">
	...
	<td width="100%" colspan="3" class="dvtCellInfo" align="left">
		<div id="editarea_{$UIKEY}">
			...
			<br>
			{if $MODULE eq 'Leads'}
			<input type="button" class="crmbutton small save" value="{$APP.LBL_SAVE_LABEL}" onclick="newcomment('{$ID}','widget')"/> 
			{else}
			<input type="button" class="crmbutton small save" value="{$APP.LBL_SAVE_LABEL}" onclick="ModCommentsCommon.addComment('{$UIKEY}', '{$ID}');"/> 
			{/if}
			{$APP.LBL_OR}
