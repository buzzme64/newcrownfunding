   ...
   <tr>
	...
	<td class="showPanelBg" valign=top width=100%>
		...
		<div class="small" style="padding:20px">
			...
			{if $MODULE eq 'Leads'}
				<table id="errorzone" class="searchUIBasic small" cellspacing="0" cellpadding="5" width="100%" border="0" align="center" style="display:none;">
					<tr>
						<td id="errormessage"></td>
						<td style="width:24px;cursor:pointer;" onclick="document.getElementById('errorzone').style.display='none';" valign="top">[x]</td>
					</tr>
				</table>
				<br/>
			{/if}
						...
						<td align=left>
										...
										<td  colspan=4 style="padding:5px">
											...
											<input title="{$APP.LBL_SAVE_BUTTON_TITLE}" accessKey="{$APP.LBL_SAVE_BUTTON_KEY}" class="crmbutton small save" onclick="this.form.action.value='Save'; displaydeleted();  if(formValidate()) {ldelim} if(AjaxDuplicateValidate('Accounts','accountname',this.form)) {ldelim} AddressSync(this.form,{$ID}); {rdelim} {rdelim}"  type="button" name="button" value="  {$APP.LBL_SAVE_BUTTON_LABEL}  " style="width:70px" >
												...
												{elseif $MODULE eq 'Leads'}
													<input title="{$APP.LBL_SAVE_BUTTON_TITLE}" accessKey="{$APP.LBL_SAVE_BUTTON_KEY}" class="crmButton small save" onclick="this.form.action.value='Save'; displaydeleted(); return validationform(); " type="button" name="button" value="  {$APP.LBL_SAVE_BUTTON_LABEL}  " style="width:70px" >
												{else}
							...
							{/if}
                                        ...
                                        {*elseif $MODULE eq 'Leads' && $header eq 'Description Information'}
											<td colspan=4 class="detailedViewHeader" style="display:none;" id="descriptionheader">
												<b>{$header}</b>
										{elseif $MODULE == 'Leads' && $header == 'LEAD STATUS'}
												<td colspan=4 class="detailedViewHeader"{* style="font-size:14px;">
												<b>{$header}</b>*}
										{elseif $MODULE eq 'Leads'}
											{if $header eq 'Description Information' || $header eq 'Contract Out' || $header eq 'Funded'}
												{if $header eq 'Description Information'}
													{assign value='descriptionheader' var=idheader}
												{elseif $header eq 'Contract Out'}
													{assign value='contractoutheader' var=idheader}
													{assign value='rowcontractout' var=idrow}
												{elseif $header eq 'Funded'}
													{assign value='fundedheader' var=idheader}
													{assign value='rowfunded' var=idrow}
												{/if}
												<td colspan=4 class="detailedViewHeader" style="display:none;" id="{$idheader}">
												<b>{$header}</b>
												{assign value=1 var=i}
											{else}
