...
-->*}
		...
		{/if}
		{if $MODULE eq 'Leads'}
			{assign var="tdstyle" value="$maindata[7]"}
		{else}
...
alt="Clear" title="Clear" LANGUAGE=javascript	onClick="this.form.{$fldname}.value=''; this.form.{$fldname}_display.value=''; return false;" align="absmiddle" style='cursor:hand;cursor:pointer'>&nbsp;
		...
		{elseif $uitype eq 11 || $uitype eq 1 || $uitype eq 13 || $uitype eq 7}
			<td data-side="toremove" width=20% class="dvtCellLabel" align=right{$tdstyle}><font color="red">{$mandatory_field}</font>{$usefldlabel} {if $MASS_EDIT eq '1'}<input type="checkbox" name="{$fldname}_mass_edit_check" id="{$fldname}_mass_edit_check" class="small" >{/if}</td>
			...
			{else}
				<td data-side="toremove" width=30% align=left class="dvtCellInfo"><input type="text" tabindex="{$vt_tab}" name="{$fldname}" id ="{$fldname}" value="{$fldvalue}" class=detailedViewTextBox onFocus="this.className='detailedViewTextBoxOn'" onBlur="this.className='detailedViewTextBox'"></td>
			{/if}
		{elseif $uitype eq 9}
		...
		{elseif $uitype eq 21 || $uitype eq 24}
			<td width=20% class="dvtCellLabel" align=right{$tdstyle}>
					<font color="red">{$mandatory_field}</font>
				{$usefldlabel} {if $MASS_EDIT eq '1'}<input type="checkbox" name="{$fldname}_mass_edit_check" id="{$fldname}_mass_edit_check" class="small" >{/if}
			</td>
			<td width=30% align=left class="dvtCellInfo">
		...
		{elseif $uitype eq 15 || $uitype eq 16  || $uitype eq '31' || $uitype eq '32'}
			<td width="20%" class="dvtCellLabel" align=right{$tdstyle}{*if $usefldlabel eq 'Lead Status'} style='font-size:14px;'{/if*}>
				<font color="red">{$mandatory_field}</font>
				{$usefldlabel} {if $MASS_EDIT eq '1'}<input type="checkbox" name="{$fldname}_mass_edit_check" id="{$fldname}_mass_edit_check" class="small" >{/if}
			</td>
			<td {if $MODULE eq 'Leads' && $uitype eq '15' && $usefldlabel eq 'Lead Status'}colspan="3"{else}width="30%"{/if} align=left class="dvtCellInfo">
			{if $MODULE eq 'Leads' && $uitype eq '15' && $usefldlabel eq 'Lead Status'}
				{assign var='existselect' value=false}
				{assign var='appnotsend' value=false}
				{foreach item=arr from=$fldvalue}
					{if $arr[2] eq 'selected'}
					{assign var='existselect' value=true}
						{if $arr[0] eq '1. App not sent' or $arr[0] eq 'Lead Pending'}
							{assign var='appnotsend' value=true}
						{/if}
					{/if}
				{/foreach}
				{assign var='currstep' value=true}
				{assign var='nextstep' value=false}
				{foreach item=arr from=$fldvalue}
					{if $existselect eq false}
						{if $arr[0] eq 'Lead Pending'}
							<input type='radio' name="{$fldname}" value="{$arr[0]}" checked="" onclick="hideviewdescription(this.value)" />
						{else}
							<input type='radio' name="{$fldname}" value="{$arr[0]}" onclick="hideviewdescription(this.value)" />
						{/if}
					{elseif $appnotsend eq true}
						{if $arr[2] eq 'selected'}
							<input type='radio' name="{$fldname}" value="{$arr[0]}" checked="" onclick="hideviewdescription(this.value)" />
						{else}
							<input type='radio' name="{$fldname}" value="{$arr[0]}" onclick="hideviewdescription(this.value)" />
						{/if}
					{else}
						{if $arr[0] eq '1. App not sent' && $existselect eq false}
							<input type='radio' name="{$fldname}" value="{$arr[0]}" checked="" onclick="hideviewdescription(this.value)" />
						{else}
							{if $arr[0] eq '10. Declined' or $arr[0] eq 'Lead Pending' or $arr[0] eq '9. Merchant Pass'}
								<input type='radio' name="{$fldname}" value="{$arr[0]}"{if $arr[2] eq 'selected'} checked=""{/if} onclick="hideviewdescription(this.value)" />
								{if $arr[2] eq 'selected'}
									{assign var='nextstep' value=false}
								{/if}
							{else}
								{if $arr[2] eq 'selected'}
									{assign var='nextstep' value=true}
									<input type='radio' name="{$fldname}" value="{$arr[0]}"{if $currstep eq true} checked=""{/if} onclick="hideviewdescription(this.value)" />
									{assign var='currstep' value=false}
								{elseif $existselect eq true}
									<input type='radio' name="{$fldname}" value="{$arr[0]}"{if $currstep eq true} checked=""{/if}{if $nextstep eq false} disabled=""{/if} onclick="hideviewdescription(this.value)" />
									{assign var='nextstep' value=false}
								{else}
									<input type='radio' name="{$fldname}" value="{$arr[0]}"{if $nextstep eq false} disabled=""{/if} onclick="hideviewdescription(this.value)" />
								{/if}
							{/if}
						{/if}
					{/if}
		...
		{elseif $uitype eq 55 || $uitype eq 255}
			...
			{/if}
			{if $uitype eq 55}
				{if $usefldlabel eq 'First Name' && $MODULE eq 'Leads'}<font color="red">{$mandatory_field}</font>{/if}{$usefldlabel}{if $MASS_EDIT eq '1'}<input type="checkbox" name="{$fldname}_mass_edit_check" id="{$fldname}_mass_edit_check" class="small" >{/if}
			...
			<td width="30%" align=left class="dvtCellInfo">
			{if $fldvalue neq ''}
				{if $uitype eq 55 && $MODULE eq 'Leads'}<span style="display: none;">{/if}
			<select name="salutationtype" class="small">
				...
				{if $uitype eq 55 && $MODULE eq 'Leads'}</span>{/if}
		...
		<div>
			<span style="display:none"><br><input name="{$fldname}" id="{$fldname}_I__" type="file" value="{$secondvalue}" tabindex="{$vt_tab}" onchange="validateFilename(this);validateFileSize(this,'{$UPLOAD_MAXSIZE}');" style="display: none;"/></span>
			<input type="hidden" name="{$fldname}_hidden" value="{$secondvalue}"/>
			<input type="hidden" name="id" value=""/>
			<input style="margin:12px 0;" type="text" id="{$fldname}_E__" name="{$fldname}" class="detailedViewTextBox" onFocus="this.className='detailedViewTextBoxOn'" onBlur="this.className='detailedViewTextBox'" value="{$secondvalue}" />
