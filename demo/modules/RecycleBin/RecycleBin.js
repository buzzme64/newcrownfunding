...
function massRemove()
{
	...
	if(confirm_status){
		var selectmodule = $('selected_module').value;
		var selectmoduletranslated =  $('selected_module_translated').value;
		if(confirm(mod_alert_arr.MSG_DELETE_CONFIRMATION + " " + count + " " + selectmoduletranslated + "?"))
		{
			$("status").style.display="inline";
			new Ajax.Request(
				'index.php',
				{
					...
					method: 'post',
					postBody: 'action=RecycleBinAjax&module=RecycleBin&mode=ajax&file=Remove&idlist='+idstring+'&selectmodule='+selectmodule+'&excludedRecords='+excludedRecords,
					onComplete: function(response)
					{
						$("status").style.display="none";
						$("modules_datas").innerHTML=response.responseText;
...
function removerecord(entityid,select_module)
{
	if(confirm(mod_alert_arr.MSG_DELETE_CONFIRMATION + " " + select_module + "?"))
	{
		$("status").style.display="inline";
		new Ajax.Request(
			'index.php',
	        {
				queue: {position: 'end', scope: 'command'},
	            method: 'post',
	            postBody: 'action=RecycleBinAjax&module=RecycleBin&mode=ajax&file=Remove&idlist='+entityid+'&selectmodule='+select_module,
		        onComplete: function(response) {
		            $("status").style.display="none";
		            $("modules_datas").innerHTML=response.responseText;
					$("search_ajax").innerHTML = '';
