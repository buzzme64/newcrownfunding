...
{include file='DetailViewSearch.tpl'}
															...
															<table border=0 cellspacing=0 cellpadding=0 width=100%>
																...
																<form action="index.php" method="post" name="DetailView" id="form">
																	...
																	{foreach key=header item=detail from=$BLOCKS}
																		...
																		{if $MODULE eq 'Leads' && $header eq 'Business Information'}
																				...
																				<td class="dvInnerHeader" colspan="4">
																					<div style="float:left;font-weight:bold;">
																						<div style="float:left;">
																							<a href="javascript:showHideStatus('tblData','aidData','themes/softed/images/');">
																								<img title="Display" alt="Display" style="border: 0px solid #000000;" src="themes/images/inactivate.gif" id="aidData">
																							</a>
																						</div>
																						<b>&nbsp;Data</b>
																					</div>
																			...
																			<div id="tblData" style="width:auto;display:none;">
																				<table class="small" cellspacing="0" cellpadding="0" width="100%" border="0"><tr><td style="padding:5px">
																		{/if}
																		{if $MODULE eq 'Leads' && $header eq 'Funded'}
																			{$ALLFUNDED}
																		{else}
																				<!-- Detailed View Code starts here-->
																				{if $MODULE eq 'Leads' && $header eq 'LEAD STATUS'}
																					<table border=0 cellspacing=0 cellpadding=0 width=100% class="small" style='font-size:12px;'>
																				{elseif $MODULE eq 'Leads' && $header eq 'Description Information'}
																					<table border=0 cellspacing=0 cellpadding=0 width=100% class="small" style='display:none;' id="description">
																				{*elseif $MODULE eq 'Leads' && $header eq 'Contract Out'}
																					<table border=0 cellspacing=0 cellpadding=0 width=100% class="small" style='display:none;' id="contractout">
																				{elseif $MODULE eq 'Leads' && $header eq 'Funded'}
																					<table border=0 cellspacing=0 cellpadding=0 width=100% class="small" style='display:none;' id="funded">*}
																				{else}
																				...
																				{if $header neq 'Comments'}
																					{if $BLOCKINITIALSTATUS[$header] eq 1}
																						...
																						{/if}
																						{if $MODULE eq 'Leads' && $header eq 'LEAD STATUS'}
																							<table border=0 cellspacing=0 cellpadding=0 width="100%" class="small" style="font-size:11px;">
																						{else}
																							<table border=0 cellspacing=0 cellpadding=0 width="100%" class="small">
																						{/if}
																								{foreach item=detail from=$detail}
																									<tr style="height:25px">
																										{foreach key=label item=data from=$detail}
																											...
																											{if $MODULE eq 'Leads'}
																												{assign var="tdstyle" value=$data.tdstyle}
																											{else}
																											...
																											{if $label ne ''}
																												...
																												{elseif $keyid eq '71' || $keyid eq '72'}<!-- Currency symbol -->
																													...
																													{elseif $MODULE eq 'Leads' && $header eq 'LEAD STATUS'}
																													<td class="dvtCellLabel" align=right width=10%><input type="hidden" id="hdtxt_IsAdmin" value={$keyadmin}></input>{$label}</td>
																													{else}
																					...
																					{/if}
																				{if $MODULE eq 'Leads' && $header eq 'OTHER INFORMATION'}
																					</td></tr></table></div>
																				{/if}
																				{/if}
																			</td>
					...
					echo vtlib_process_widget($this->_tpl_vars['CUSTOM_LINK_DETAILVIEWWIDGET'], $this->_tpl_vars);
											...
											<td width=22% valign=top style="border-left:1px dashed #cccccc;padding:13px">
																...
																<tr>
																	<td align="left" style="padding-left:10px;">
																		{foreach key=index item=email from=$EMAILS}
																			<input type="hidden" name="email_{$index}" value="{$email}"/>
																		{/foreach}
																		<a href="javascript:void(0);" class="webMnu" onclick="sendemailpdf({$ID})"><img src="{'sendmail.png'|@vtiger_imageurl:$THEME}" hspace="5" align="absmiddle"  border="0"/></a>&nbsp;
																		<a href="javascript:void(0);" class="webMnu" onclick="sendemailpdf({$ID})">{$APP.LBL_SENDMAIL_BUTTON_LABEL} PDF app</a>
																	</td>
																</tr>
																<tr>
																<td td align="left" style="padding-left:10px;">
																<a href="javascript:void(0);" class="webMnu" onclick="sendemailbusinesspdf({$ID})"><img src="{'sendmail.png'|@vtiger_imageurl:$THEME}" hspace="5" align="absmiddle"  border="0"/></a>&nbsp;
																		<a href="javascript:void(0);" class="webMnu" onclick="sendemailbusinesspdf({$ID})">Business loan {$APP.LBL_SENDMAIL_BUTTON_LABEL} PDF app</a>
																	</td>
																	</tr>
																	<tr>
																<td td align="left" style="padding-left:10px;">
																<a href="javascript:void(0);" class="webMnu" onclick="sendemailpdffax({$ID})"><img src="{'sendmail.png'|@vtiger_imageurl:$THEME}" hspace="5" align="absmiddle"  border="0"/></a>&nbsp;
																		<a href="javascript:void(0);" class="webMnu" onclick="sendemailpdffax({$ID})">Fax loan PDF app</a>
																	</td>
																	</tr>
																	<tr>
																<td td align="left" style="padding-left:10px;">
																<a href="javascript:void(0);" class="webMnu" onclick="sendemailbusinesspdffax({$ID})"><img src="{'sendmail.png'|@vtiger_imageurl:$THEME}" hspace="5" align="absmiddle"  border="0"/></a>&nbsp;
																		<a href="javascript:void(0);" class="webMnu" onclick="sendemailbusinesspdffax({$ID})">Fax Business loan PDF app</a>
																	</td>
																	</tr>
																{/if}
															{/if}
												...
												{if $CUSTOM_LINKS && $CUSTOM_LINKS.DETAILVIEWBASIC}
													<table width="100%" border="0" cellpadding="5" cellspacing="0">
														{foreach item=CUSTOMLINK from=$CUSTOM_LINKS.DETAILVIEWBASIC}
															...
															{if $customlink_label neq 'LBL_ADD_NOTE'}
															<tr>
																<td align="left" style="padding-left:10px;">
												...
												{/if}
{if $MODULE eq 'Leads'}{$LAST7COMMENT}{/if}
