...
<tr><td>
	...
	<!-- Buttons -->
    		<td style="padding-right:20px" nowrap>
		        ...
		        {if $IS_ADMIN eq 'true'}
				<input class="crmbutton small delete" type="button" onclick ="massRemove();" value="{$MOD.LBL_MASS_DELETE}">
	...
	<tr bgcolor=white onMouseOver="this.className='lvtColDataHover'" onMouseOut="this.className='lvtColData'" id="row_{$entity_id}">
        ...
        <td >{$data}</td>
		{/foreach}
		<td class="samll"> <a href="javascript:;" onclick='restore({$entity_id},"{$SELECTED_MODULE}");'>{$MOD.LNK_RESTORE}</a> | <a href="javascript:;" onclick='removerecord({$entity_id},"{$SELECTED_MODULE}");'>{$MOD.LNK_DELETE}</a></td>
	</tr>
	{foreachelse}
	<tr>
