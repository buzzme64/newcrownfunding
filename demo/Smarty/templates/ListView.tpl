...
<table border=0 cellspacing=0 cellpadding=0 width=98% align=center>
	...
	<!-- Lead Search-->
	 <!-- SIMPLE SEARCH -->
<div id="searchAcc" style="display: {*if $MODULE neq 'Leads'*}block{*else}none{/if*};position:relative;">
...
<table width="100%" cellpadding="5" cellspacing="0"  class="searchUIBasic small" align="center" border=0>
	<tr>
		...
		<td class="small"{if $MODULE eq 'Leads'} style="text-align: center; width:125px;"{/if}><input type="text"  class="txtBox" style="width:120px;" name="search_text"></td>
		<td class="small" nowrap {if $MODULE eq 'Leads'}style="display:none;"{/if}><b>{$APP.LBL_IN}</b>&nbsp;</td>
		<td class="small" nowrap{if $MODULE eq 'Leads'} style="display:none;"{/if}>
			<div id="basicsearchcolumns_real">
			<select name="search_field" id="bas_searchfield" class="txtBox" style="width:150px;{if $MODULE eq 'Leads'}display:none;{/if}">
		...
		<td colspan="7" align="center" class="small" style="display: none;">
			<table border=0 cellspacing=0 cellpadding=0 width=100%>
				<tr>
								...
								{*if $MODULE eq 'Leads' && $CURRENT_USER_ID neq 1}
									<table>
										<tr>
											<td style="background-color: #03ff00; font-size: 18px; font-weight: bold; padding: 10px;">Leads Pending: <a href="index.php?module=Leads&action=ListView&leadpend=0">{$COUNTLEAD}</a></td>
										</tr>
									</table>
								{/if*}
<!-- ADVANCED SEARCH -->
