...
//BNJ added cf_735 below
	...
	$query="select crm.smownerid, l.leadid, l.firstname, l.lastname, lcf.cf_641 dba,lcf.cf_734,crm.modifiedtime , lcf.cf_735, lcf.cf_736 as stat_date,na.timeadd as nstat_date
	from vtiger_leaddetails l
	inner join vtiger_leadscf lcf on l.leadid=lcf.leadid
	inner join vtiger_crmentity crm on l.leadid=crm.crmid
	inner join vtiger_leads_appsout na on l.leadid=na.leadid
	where l.leadstatus='2. App Out' ";
	//if ($startdate!='') $query.=" and changeassigned  >='2015-01-23' -   INTERVAL 5 DAY";
	//if ($enddate!='') $query.=" and changeassigned  <='2015-02-23' + INTERVAL 1 DAY ";
	//if ($startdate!='') $query.=" and  crm.modifiedtime    >='".$startdaten." 00:00:00'";
	//if ($enddate!='') $query.=" and crm.modifiedtime <='".$enddaten." 00:00:00'";
	$query.=" and na.timeadd   >='".$startdaten."'";
	$query.=" and na.timeadd  <='".$enddaten."'";
...
<form action="index.php?module=Report&action=appoutreport" method="POST" name="search">
...
foreach($restable as $key=>$value){
	...
	<td onmouseover=\"vtlib_listview.trigger('cell.onmouseover', $(this))\" onmouseout=\"vtlib_listview.trigger('cell.onmouseout', $(this))\">".$value1['priority']."</td>
	</tr>";
